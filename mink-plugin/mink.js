...
chrome.browserAction.onClicked.addListener(function (tab) {
   ...
   if (scheme !== 'http') {
      if (debug) {
         console.log('Invalid scheme for Mink: ' + scheme);
      }
      return;
   }
   ...
   chrome.storage.local.get('timemaps', function (items) {
      if (items.timemaps && items.timemaps[tab.url]) {
         if (debug) {
            ...
            console.log(items.timemaps[tab.url]);
...
function showMinkBadgeInfoBasedOnProcessingState(tabid) {
   chrome.storage.local.get('disabled', function (items) {
      ...
      var cb = function () {
         setBadgeTextBasedOnBrowserActionState(tabid);
      };
...
chrome.runtime.onMessage.addListener(
   function (request, sender, sendResponse) {
      ...
      if (request.method == 'store') {
         ...
         if (debug) {
            console.log('Message sent using chrome.runtime not caught: ' + request.method);
         }
      }
   }
);
...
function fetchTimeMap(uri, tabid) {
   ...
   $.ajax({
      ...
      if (xhr.status === 404) {
         if (debug) {
            console.log('querying secure FAILED, Display zero mementos interface');
         }
...
function setBadgeTitle(newTitle, tabid) {
   if (debug) {
      console.warn('setting badge title');
   }
...
function stopWatchingRequests() {
   if (debug) {
      console.log('stopWatchingRequests() executing');
   }
...
function stopWatchingRequests_blacklisted() {
   if (debug) {
      console.log('stopWatchingRequests_blacklisted() executing');
   }
...
chrome.contextMenus.create({
   ...
   if (err) {
      console.log('error creating second contextmenu');
   }
});
